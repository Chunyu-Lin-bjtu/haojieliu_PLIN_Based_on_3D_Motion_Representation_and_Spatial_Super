...
parser.add_argument('--epochsample', default=24000, type=int, metavar='N',
                    help='number of total epochs to run (default: 11)')
parser.add_argument('--mgpus', action="store_true",default=False,help='use multi-gpus')
...
args.result = os.path.join('.', 'results_experiment_all/mode-time=2019-12-26@20000')
writer = SummaryWriter('logs/logs_2')
...
def train(mode, args, loader, model, optimizer, logger, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        #     rgb1 = batch_data['rgb']
        #     rgb1 = rgb1.data.cpu().numpy()
        #     rgb1 = np.transpose(rgb1, (0, 2, 3, 1))    
        #     rgb1 = rgb1[0].astype('uint8')
        #     plt.figure('22')
        #     plt.imshow(rgb1)
        #     plt.show()
        #     plt.savefig('rgb1.png', bbox_inches='tight')
        #     input('---------rgb1.png-----------')
        ...
        # print('pc.shape',batch_data['pc0'].shape)
        ...
        if mode == 'train':
            ...
            if 'sparse' in args.train_mode:
                ...
                cdloss = depth_cdloss(pred,gt)
                # bc_loss = bc(precise_depth,batch_data['edge'])
                # loss_lidar = depth_criterion(lidar_out, gt)
                # loss_rgb = depth_criterion(precise_depth, gt)
                # loss_guide = depth_criterion(global_features, gt)
                # mask = (gt < 1e-3).float()
            ...
            # loss = depth_loss + 0.01*loss_lidar + 0.01*loss_rgb + 0.01*loss_guide
            loss = depth_loss + cdloss
            # loss = depth_loss + 1*cdloss + 0.01*bc_loss
            if step % 100 == 0:
                # print('loss',depth_loss,loss_guide)
                print('loss',depth_loss,loss)

            writer.add_scalar('loss', loss, cont)
            # writer.add_scalar('bc_loss', bc_loss, cont)
            # writer.add_scalar('cdloss', cdloss, cont)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def main():
    ...
    # ## print("=> model transferred to a GPU.")
    # print("=> creating model and optimizer...")
    # model = DepthCompletionNet(args)
    # model.cuda()
    ...
    if not is_eval:
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2, shuffle=False,num_workers=args.workers, pin_memory=True, sampler=None)
