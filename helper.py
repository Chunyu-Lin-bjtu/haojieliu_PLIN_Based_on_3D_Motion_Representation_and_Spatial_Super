...
class logger:
    ...
    def conditional_print(self,split,i,epoch,lr,n_set,blk_avg_meter,avg_meter):
        if (i + 1) % self.args.print_freq == 0:
            ...
            # )
            print('{split} Epoch: {0} [{1}/{2}]\t'
                  'RMSE={blk_avg.rmse:.2f}({average.rmse:.2f}) '
                  'MAE={blk_avg.mae:.2f}({average.mae:.2f}) '
                  'iRMSE={blk_avg.irmse:.2f}({average.irmse:.2f})\n\t'.format(
                  epoch, i+1, n_set, blk_avg=blk_avg, average=avg,
                  split=split.capitalize())
            )
            blk_avg_meter.reset()
    ...
    #     if mode == 'val' or mode == 'eval':
    #         skip = 300
    #         temp = [200,400,600,1814,3538,4057,4208,4386,4510,4661,5300]
    #         if i == 200:
    #             self.img_merge = vis_utils.merge_into_row(ele, pred)
    #         elif i in temp :
    #             row = vis_utils.merge_into_row(ele, pred)
    #             self.img_merge = vis_utils.add_row(self.img_merge, row)
    #             # filename = str(i)+str(i)+self._get_img_comparison_name(mode, epoch) 
    #             # vis_utils.save_image(row, filename)
    #         elif i ==5400:
    #             # self.img_merge = vis_utils.add_row(self.img_merge, row)
    #             filename =self._get_img_comparison_name(mode, epoch) 
    #             vis_utils.save_image(self.img_merge, filename)
    #         # elif i in temp:
    #         #     row = vis_utils.merge_into_row(ele, pred)
    #         #     vis_utils.save_image(self.img_merge, str(i)+'.png')
    #         #     self.img_merge = vis_utils.add_row(self.img_merge, row)
    #         # elif i > 6000:
    #         #     filename = self._get_img_comparison_name(mode, epoch)
    #         #     vis_utils.save_image(self.img_merge, filename)
...
def get_folder_name(args):
    ...
    if args.use_pose:
        ...
        prefix = "mode-"
    return os.path.join(args.result,prefix + 'time={}'.format(current_time))
